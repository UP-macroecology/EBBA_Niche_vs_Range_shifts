...
# Sensitivity analyses: only quantify niche metrics for newly colonised range 
...
data_dir_dz <- file.path("/import", "ecoc9z", "data-zurell", "zurell", "EBBA_niche_range_shifts")
...
plots_dir_dz <- file.path("/import", "ecoc9z", "data-zurell", "zurell", "EBBA_niche_range_shifts", "plots")


# folder for niche dynamics plots:
plots_dir <- file.path(plots_dir_dz, "niche_dynamics_species", paste0("BBS_niche_dyn_novelRange_bg_", ifelse(bg_spec, "spec", "US"), "_hist", 
                                                                   ifelse(all(hist_years == 1980:1983), "81-83", "88-90")))
if(!dir.exists(plots_dir)){dir.create(plots_dir, recursive = TRUE)}

# results table:
results_file <- file.path(data_dir_dz, paste0("BBS_niche_shift_results_novelRange_bg_", ifelse(bg_spec, "spec", "US"),  "_hist",
...
# species selection: 
sel_species <- read.csv(file = file.path(data_dir_dz, "BBS_stability_PCA_contUS_BL22.csv")) %>%
  filter(stability >= 0.5) %>% 
  pull(species) %>% 
  sort # 234

# BBS data, only selected species:
load(file = file.path(data_dir_dz, paste0("BBS_prep_steps1-4_hist", ifelse(all(hist_years == 1980:1983), "81-83", "88-90"), ".RData"))) # output of 2_1_BBS_species_filtering_1-4.R
...
niche_shift_df <- foreach(s = 1:length(sel_species_final),
                          ...
                          .inorder = FALSE) %dopar% {
                            ...
                            # species records (presences and true absences):
                            spec_rec <- spec_rec_novel <- BBS_rec %>%
                              filter(species == spec)
                            
                            # identify the newly colonised cells (cells that are occupied in recent time period but not in historic time period) and set presence of all other cells to 0
                            spec_rec_novel[spec_rec$RTENO %in% spec_rec$RTENO[spec_rec$pres==1][spec_rec$RTENO[spec_rec$pres==1] %in% spec_hist$RTENO[spec_hist$pres==1]],]$pres <- 0
