...
niche_results_BBS_novelRange <- read.csv(file.path(data_dir_BBS, "BBS_niche_shift_results_novelRange_bg_spec_hist81-83.csv"))
...
niche_results_EBBA_novelRange <- read.csv(file.path(data_dir_EBBA, "EBBA_niche_shift_results_novelRange_bg_spec_230923.csv"))
...
#------------- Niche dynamic boxplots ---------------
# ---- without abandonment and pioneering -----------
...
# exclude niche abandonment/pioneering and rescale: niche unfilling + niche stability + niche expansion = 1
# BBS
niche_results_BBS_woNovelClim <- niche_results_BBS %>%
  select(!c(niche_abandonment_std, niche_pioneering_std))
sum_niche_BBS_analogClim <- niche_results_BBS_woNovelClim %>% 
  select(starts_with("niche_")) %>%
  rowSums()
niche_results_BBS_woNovelClim$niche_stability_std <- niche_results_BBS_woNovelClim$niche_stability_std / sum_niche_BBS_analogClim
niche_results_BBS_woNovelClim$niche_unfilling_std <- niche_results_BBS_woNovelClim$niche_unfilling_std / sum_niche_BBS_analogClim
niche_results_BBS_woNovelClim$niche_expansion_std <- niche_results_BBS_woNovelClim$niche_expansion_std / sum_niche_BBS_analogClim
# EBBA
niche_results_EBBA_woNovelClim <- niche_results_EBBA %>%
  select(!c(niche_abandonment_std, niche_pioneering_std))
sum_niche_EBBA_analogClim <- niche_results_EBBA_woNovelClim %>% 
  select(starts_with("niche_")) %>%
  rowSums()
niche_results_EBBA_woNovelClim$niche_stability_std <- niche_results_EBBA_woNovelClim$niche_stability_std / sum_niche_EBBA_analogClim
niche_results_EBBA_woNovelClim$niche_unfilling_std <- niche_results_EBBA_woNovelClim$niche_unfilling_std / sum_niche_EBBA_analogClim
niche_results_EBBA_woNovelClim$niche_expansion_std <- niche_results_EBBA_woNovelClim$niche_expansion_std / sum_niche_EBBA_analogClim


# join niche and range shift results, convert to long format:
niche_range_df_BBS <- niche_results_BBS_woNovelClim %>% 
  left_join(range_results_BBS, by = "species", suffix = c("_niche", "_range")) %>% 
  pivot_longer(cols = ends_with("_std"),
               names_to = c("category", "metric"), names_pattern = "(.*)_(.*)_",
               values_to = "value") %>% 
...
# join niche and range shift results, convert to long format:
niche_range_df_EBBA <- niche_results_EBBA_woNovelClim %>% 
  left_join(range_results_EBBA, by = "species", suffix = c("_niche", "_range")) %>% 
  pivot_longer(cols = ends_with("_std"),
               names_to = c("category", "metric"), names_pattern = "(.*)_(.*)_",
               values_to = "value") %>% 
...
labelsdat <- tibble(category = c(rep("niche", 3), rep("range", 3)),
                    metric = factor(rep(c("unfilling", "stability", "expansion"),2),
                                    levels = c("unfilling", "stability", "expansion")),
                    spec_cons_BBS = spec_cons_BBS,
                    spec_cons_EBBA = spec_cons_EBBA,
                    ypos_higher = 110)
...
# save plot:
pdf(file = file.path("plots", "dynamics_boxplots", "EBBA_boxplot_dynamics_bg_spec_woNovelClim.pdf"),
    height = 3, width = 5.5)
p_EBBA
dev.off()

pdf(file = file.path("plots", "dynamics_boxplots", "BBS_boxplot_dynamics_bg_spec_hist81-83_woNovelClim.pdf"),
    height = 3, width = 5.5)
p_BBS
dev.off()
...
#------------- Niche dynamic boxplots ---------------
# ---- only for novel range (and without abandonment and pioneering) -----------

## niche BBS:
niche_test_sign_BBS_novel <- niche_results_BBS_novelRange %>% 
  dplyr::select(c(species, matches("cons_p_"))) %>%
  summarise(across(.cols = where(is.numeric), 
                   .fns = ~length(which(.x <= 0.05)), 
                   .names = "{.col}_n_sig")) %>% 
...
## Niche EBBA:
niche_test_sign_EBBA_novel <- niche_results_EBBA_novelRange %>% 
  dplyr::select(c(species, matches("cons_p_"))) %>%
  summarise(across(.cols = where(is.numeric), 
                   .fns = ~length(which(.x <= 0.05)), 
                   .names = "{.col}_n_sig")) %>% 
...
# BBS species (%) with significant niche tracking and range lagging
spec_cons_BBS <- c(# "", # niche abandonment
  niche_test_sign_BBS_novel$cons_p_U_A_n_sig, # niche unfilling
  niche_test_sign_BBS_novel$cons_p_S_A_n_sig, # niche stability
  niche_test_sign_BBS_novel$cons_p_E_A_n_sig, # niche expansion
  # "", # niche pioneering,
  niche_test_sign_BBS$cons_p_U_A_n_sig, # range unfilling
  niche_test_sign_BBS$cons_p_S_A_n_sig, # range stabiliy
  niche_test_sign_BBS$cons_p_E_A_n_sig # range expansion
)

# EBBA species (%) with significant niche tracking and range lagging
spec_cons_EBBA <- c(# "", # niche abandonment
  niche_test_sign_EBBA_novel$cons_p_U_A_n_sig, # niche unfilling
  niche_test_sign_EBBA_novel$cons_p_S_A_n_sig, # niche stability
  niche_test_sign_EBBA_novel$cons_p_E_A_n_sig, # niche expansion
  # "", # niche pioneering,
  niche_test_sign_EBBA$cons_p_U_A_n_sig, # range unfilling
  niche_test_sign_EBBA$cons_p_S_A_n_sig, # range stabiliy
  niche_test_sign_EBBA$cons_p_E_A_n_sig # range expansion
)

# exclude niche abandonment/pioneering and rescale: niche unfilling + niche stability + niche expansion = 1
# BBS
niche_results_BBS_novel <- niche_results_BBS_novelRange %>%
  select(!c(niche_abandonment_std, niche_pioneering_std))
sum_niche_BBS_analogClim <- niche_results_BBS_novel %>% 
  select(starts_with("niche_")) %>%
  rowSums()
niche_results_BBS_novel$niche_stability_std <- niche_results_BBS_novel$niche_stability_std / sum_niche_BBS_analogClim
niche_results_BBS_novel$niche_unfilling_std <- niche_results_BBS_novel$niche_unfilling_std / sum_niche_BBS_analogClim
niche_results_BBS_novel$niche_expansion_std <- niche_results_BBS_novel$niche_expansion_std / sum_niche_BBS_analogClim

niche_results_BBS <- niche_results_BBS %>%
  select(!c(niche_abandonment_std, niche_pioneering_std))
sum_niche_BBS_analogClim <- niche_results_BBS %>% 
  select(starts_with("niche_")) %>%
  rowSums()
niche_results_BBS$niche_stability_std <- niche_results_BBS$niche_stability_std / sum_niche_BBS_analogClim
niche_results_BBS$niche_unfilling_std <- niche_results_BBS$niche_unfilling_std / sum_niche_BBS_analogClim
niche_results_BBS$niche_expansion_std <- niche_results_BBS$niche_expansion_std / sum_niche_BBS_analogClim


# EBBA
niche_results_EBBA_novel <- niche_results_EBBA_novelRange %>%
  select(!c(niche_abandonment_std, niche_pioneering_std))
sum_niche_EBBA_analogClim <- niche_results_EBBA_novel %>% 
  select(starts_with("niche_")) %>%
  rowSums()
niche_results_EBBA_novel$niche_stability_std <- niche_results_EBBA_novel$niche_stability_std / sum_niche_EBBA_analogClim
niche_results_EBBA_novel$niche_unfilling_std <- niche_results_EBBA_novel$niche_unfilling_std / sum_niche_EBBA_analogClim
niche_results_EBBA_novel$niche_expansion_std <- niche_results_EBBA_novel$niche_expansion_std / sum_niche_EBBA_analogClim

niche_results_EBBA <- niche_results_EBBA %>%
  select(!c(niche_abandonment_std, niche_pioneering_std))
sum_niche_EBBA_analogClim <- niche_results_EBBA %>% 
  select(starts_with("niche_")) %>%
  rowSums()
niche_results_EBBA$niche_stability_std <- niche_results_EBBA$niche_stability_std / sum_niche_EBBA_analogClim
niche_results_EBBA$niche_unfilling_std <- niche_results_EBBA$niche_unfilling_std / sum_niche_EBBA_analogClim
niche_results_EBBA$niche_expansion_std <- niche_results_EBBA$niche_expansion_std / sum_niche_EBBA_analogClim


# join niche and range shift results, convert to long format:
names(niche_results_BBS_novel) <- sub('niche','novel',names(niche_results_BBS_novel))
niche_df_BBS_novel <- niche_results_BBS_novel %>% 
  pivot_longer(cols = ends_with("_std"),
               names_to = c("category", "metric"), names_pattern = "(.*)_(.*)_",
               values_to = "value") %>% 
  mutate(metric = factor(metric, levels = c("abandonment", "unfilling", "stability", "expansion", "pioneering")))

niche_df_BBS <- niche_results_BBS %>% 
  pivot_longer(cols = ends_with("_std"),
               names_to = c("category", "metric"), names_pattern = "(.*)_(.*)_",
               values_to = "value") %>% 
  mutate(metric = factor(metric, levels = c("abandonment", "unfilling", "stability", "expansion", "pioneering")))

niche_df_BBS <- rbind(niche_df_BBS_novel, niche_df_BBS)

# join niche and range shift results, convert to long format:
names(niche_results_EBBA_novel) <- sub('niche','novel',names(niche_results_EBBA_novel))
niche_df_EBBA_novel <- niche_results_EBBA_novel %>% 
  pivot_longer(cols = ends_with("_std"),
               names_to = c("category", "metric"), names_pattern = "(.*)_(.*)_",
               values_to = "value") %>% 
  mutate(metric = factor(metric, levels = c("abandonment", "unfilling", "stability", "expansion", "pioneering")))

niche_df_EBBA <- niche_results_EBBA %>% 
  pivot_longer(cols = ends_with("_std"),
               names_to = c("category", "metric"), names_pattern = "(.*)_(.*)_",
               values_to = "value") %>% 
  mutate(metric = factor(metric, levels = c("abandonment", "unfilling", "stability", "expansion", "pioneering")))

niche_df_EBBA <- rbind(niche_df_EBBA_novel, niche_df_EBBA)


# data set for labels:
labelsdat <- tibble(category = c(rep("novel", 3), rep("niche", 3)),
                    metric = factor(rep(c("unfilling", "stability", "expansion"),2),
                                    levels = c("unfilling", "stability", "expansion")),
                    spec_cons_BBS = spec_cons_BBS,
                    spec_cons_EBBA = spec_cons_EBBA,
...
# plot:
p_EBBA <- ggplot(niche_df_EBBA, aes(x = category, y = value*100, fill = metric)) + # aes given inside ggplot() necessary for geom_text
  geom_boxplot(lwd = 0.1, outlier.size = 0.7, outlier.colour = "grey30", width = 0.9,
               position = position_dodge2(width = 1, preserve = "single")) +
  #facet_grid(~ category, scales = "free") +
  theme_bw() +
...
p_BBS <- ggplot(niche_df_BBS, aes(x = category, y = value*100, fill = metric)) + # aes given inside ggplot() necessary for geom_text
  geom_boxplot(lwd = 0.1, outlier.size = 0.7, outlier.colour = "grey30", width = 0.9,
               position = position_dodge2(width = 1, preserve = "single")) +
  #facet_grid(~ category, scales = "free") +
  theme_bw() +
...
# save plot:
pdf(file = file.path("plots", "dynamics_boxplots", "EBBA_boxplot_dynamics_bg_spec_novelRange.pdf"),
    height = 3, width = 5.5)
p_EBBA
dev.off()

pdf(file = file.path("plots", "dynamics_boxplots", "BBS_boxplot_dynamics_bg_spec_hist81-83_novelRange.pdf"),
    height = 3, width = 5.5)
p_BBS
dev.off()
