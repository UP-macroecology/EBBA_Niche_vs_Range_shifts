...
knitr::opts_knit$set(root.dir = ".\EBBA_NichevsRangeShift_2022")
...
Since this code accompanies the Breeding Bird Survey (EBBA) study, we process the data in two alternative ways: i) we crop the global data at the original resolution to the extent of EBBA survey grid and ii) we reproject the data to a coarser resolution of 50 km (because this matches the size of cells in the EBBA grid) and crop it to the extent of the EBBA grid.
...
target_path <- file.path(".") # "." = placeholder for the current working directory
...
EBBA2_Grid <- file.path("./Data", "input_data", "EBCC", "EBBA2", "ebba2_grid50x50_v1")

# read in rasterized EBBA2 grid

# project mask from lat/lon to equal area projection of the two final grids:
input <- file.path("./Data", "input_data", "Europe_ebba_grid_50km.tif")
...
# loop through the resolutions: #we only need 1 resolution for EBBA project, loop is holdover from origninal code, works fine though.
...
We can now use these layers to calculate `bioclim` variables for the selected time period covering the EBBA sampling grid at 50km resolution.
...
for(res in resolutions){
  ...
  # list monthly CHELSA data for europe 2009-2018 (repeated with 1981-1990 data too)
...
With the data on the data share, we can now crop and project the data to match the EBBA study.
...
# years for which data should be extracted-No data beyond 2015, need to download simulated climate layers to bring up to 2018:
years <- c(1980:2015)
...
for(lu in lu_classes){
  ...
  files <- list.files(file.path(luh2_data_path), full.names = TRUE, pattern = paste0(lu, ".*luh2.tif$")) # name contains land use class and "luh2.tif" but not the .aux extension ($)
