...
##Supplement bioclims with LUH2 data from middle years - Investigated but discarded at time of departure. Long-term averages should be used instead of midpoint years if LUH2 use is picked up again.
...
Bioclim80s <- terra::mask(Bioclim80s,KeepCountries,inverse=F,touches=T)##Removes rough edges around coastlines left over from previous mask, also all other raster cells that had a centroid not over land ... maybe benefit, but also maybe problem?

Bioclim10s <- terra::mask(Bioclim10s,MaskCountries,inverse=T)
Bioclim10s <- terra::mask(Bioclim10s,KeepCountries,inverse=F,touches=T)##Removes rough edges around coastlines left over from previous mask, also all other raster cells that had a centroid not over land ... maybe benefit, but also maybe problem?
