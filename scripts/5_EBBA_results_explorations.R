...
pdf(file = file.path(plots_dir, "EBBA_correlation_niche_range_dynamics.pdf"),
    height = 5, width = 8)
...
EBBA1_cells <- read_sf(file.path(EBBA_data_dir, "EBBA_change.shp")) %>% # output of 1_prep_EBBA_data.R
  select(-species, -Change) %>% 
  distinct(cell50x50, .keep_all = TRUE) # keep geometry; would be the same when using EBBA2
# centroids

EBBA_change_grid <- read_sf(file.path(EBBA_data_dir, "ebba2_grid50x50_change_v1", "ebba2_grid50x50_change_v1.shp")) # EPSG:3035
...
EBBA1_climate_data <- EBBA1_cells %>% 
...
EBBA2_climate_data <- EBBA1_cells %>% # same as EBBA2 cells
  vect %>% # convert to terra object
  cbind(terra::extract(biovars_rec_rast, y = .)) %>% # add bioclim data
  as.data.frame %>% 
  select(-ID)
...
p_biplot2 <- factoextra::fviz_pca_biplot(pca.env, repel = TRUE,
                            ...
                            col.ind = c(rep("historic", nrow(EBBA1_climate_data)),
                                        rep("recent", nrow(EBBA2_climate_data))),  # Individuals color
                            label = "var",
                            palette = c( "red", "blue"),
                            pointsize = 1.5,
                            alpha.ind = 0.1,
...
# match with EBBA cells:
map_sf <- EBBA_change_grid %>% 
  left_join(map_df, by = c("cell50x50_" = "cell50x50"))
...
p_climate_change
# save plot:
pdf(file = file.path(plots_dir, "EBBA_climate_change_PCA_plots.pdf"),
    height = 5, width = 20)
p_climate_change
dev.off()
...
# save plots:
pdf(file = file.path(plots_dir, paste0("EBBA_range_shift_direction_env_", env_background_species_specific, ".pdf")),
    height = 5, width = 6)
dir_shift_p
dev.off()

pdf(file = file.path(plots_dir, paste0("EBBA_range_shift_direction2_env_", env_background_species_specific, ".pdf")),
    height = 5, width = 6)
dir_shift_p2
dev.off()

pdf(file = file.path(plots_dir, paste0("EBBA_range_shift_direction_hist_env_", env_background_species_specific, ".pdf")),
    height = 5, width = 6)
dir_shift_hist
dev.off()
