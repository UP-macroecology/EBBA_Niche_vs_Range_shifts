...
plots_dir <- file.path(plots_dir_dz, "niche_dynamics_species", paste0("EBBA_niche_dyn_novelRange_bg_", ifelse(bg_spec, "spec", "EBBA")))
if(!dir.exists(plots_dir)){dir.create(plots_dir, recursive = TRUE)}

# results table:
results_file <- file.path(data_dir_dz, paste0("EBBA_niche_shift_results_novelRange_bg_", ifelse(bg_spec, "spec", "EBBA"),"_230923", ".csv"))
...
load(file = file.path(data_dir_dz, "EBBA1_EBBA2_prep_steps1-4_final.RData")) # output of 2_1_EBBA_species_filtering_1-4.R
...
niche_shift_df <- foreach(s = 1:length(sel_species),
        ...
        .errorhandling = "remove") %dopar% {
          ...
          if(bg_spec){
            ...
            novelRange <- EBBA2_pres_sf$cell50x50[EBBA2_pres_sf$cell50x50 %in% EBBA1_pres_sf$cell50x50]
            EBBA2_pres_novel_sf <- EBBA2_pres_sf %>%
              filter(cell50x50 %in% novelRange)
            
            
            # buffer to determine cells used as absences:
            EBBA2_spec_buffer <- EBBA2_pres_novel_sf %>% 
              st_buffer(dist = 500000) %>% 
              st_union
            ...
            EBBA2_spec_niche_data <- EBBA_cells %>% 
              mutate(species_occ = ifelse(cell50x50 %in% EBBA2_pres_novel_sf$cell50x50, 1, 
