...
##coerce loop lists into dataframe to simplify data handling##
df <- melt(TestMetrics)#Coerce to dataframe
...
df <- subset(df,df$Test=="SimShiftNA")##Restrict to one test type to remove duplication
...
df$RangeShiftMagnitude <- df$RangeShiftMagnitude/1000#Convert meters to km for range shift
...
df$Groups <- round(seq(from=1,to=5,length.out=nrow(df)),digits=0)##Add arbitrary group ID's for playing around with, as binomials are alphabetized, generated ID's where adjacent rows more likely to be in the same group, so groups are slightly less arbitrary and likely to contain species in the same genus
