...
if(env_background_species_specific){
  plots_dir <- file.path(EBBA_data_dir, "plots", "EBBA_1_2_niche_dyn_env_species_spec")
} else {
  plots_dir <- file.path(EBBA_data_dir, "plots", "EBBA_1_2_niche_dyn_env_EBBA")
}
...
# output file:
if(env_background_species_specific){
  results_file <- file.path(EBBA_data_dir, "niche_shift_results_env_species_spec.csv")
} else {
  results_file <- file.path(EBBA_data_dir, "niche_shift_results_env_EBBA.csv")
}
...
if(env_background_species_specific == FALSE){
  ...
  pca.env <- dudi.pca(rbind(EBBA1_climate_data, EBBA2_climate_data)[,2:20],
                      scannf = FALSE,
                      nf = 2) # number of axes
  ...
  scores.clim.EBBA1 <- suprow(pca.env, EBBA1_climate_data[,2:20])$li
  # PCA scores for whole EBBA2 area:
  scores.clim.EBBA2 <- suprow(pca.env, EBBA2_climate_data[,2:20])$li
...
niche_shift_df <- foreach(s = 1:length(sel_species),
        ...
        .verbose = TRUE) %dopar% {
          ...
          if(env_background_species_specific){
            ...
            # PCA scores for EBBA1 species distribution:
            scores.sp.EBBA1 <- suprow(pca.env, EBBA1_spec_niche_data[,3:21])$li 
            
            # PCA scores for EBBA2 species distribution:
            scores.sp.EBBA2 <- suprow(pca.env, EBBA2_spec_niche_data[,3:21])$li 
          ...
          niche_shift_spec_df$niche_centroid_hist_X <- median(scores.sp.EBBA1[,1]) 
