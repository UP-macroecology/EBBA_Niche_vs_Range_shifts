...
if(env_background_species_specific){
  plots_dir <- file.path(EBBA_data_dir, "plots", "EBBA_1_2_range_dyn_env_species_spec_change")
} else {
  plots_dir <- file.path(EBBA_data_dir, "plots", "EBBA_1_2_range_dyn_env_EBBA_change")
}
...
# output file:
if(env_background_species_specific){
  results_file <- file.path(EBBA_data_dir, "range_shift_results_env_species_spec_change.csv")
} else {
  results_file <- file.path(EBBA_data_dir, "range_shift_results_env_EBBA_change.csv")
}
...
EBBA1_cells %>% 
  ...
  st_write(file.path(EBBA_data_dir, "EBBA_comparable_buffer20km_change.shp"), append = FALSE)

gdalUtilities::gdal_rasterize(src_datasource = file.path(EBBA_data_dir, "EBBA_comparable_buffer20km_change.shp"),
                              dst_filename = file.path(EBBA_data_dir, "EBBA_mask_ecospat_change20km.tif"),
                              burn = 1,
...
EBBA_mask_sp <- rast(file.path(EBBA_data_dir, "EBBA_mask_ecospat_change20km.tif")) %>%
  as.polygons %>% 
  st_as_sf %>% 
