...
biovars_hist_rast <- rast(list.files(file.path(data_dir, "Bioclim_1981_1990"),
                                     pattern = "50km.tif$",
                                     full.names = TRUE))
...
biovars_rec_rast <- rast(list.files(file.path(data_dir, "Bioclim_2009_2018"),
                                    pattern = "50km.tif$",
                                    full.names = TRUE))
# join EBBA cells and corresponding climate data:
...
bioclim1_hist <- rast(file.path(data_dir, "Bioclim_1981_1990", "CHELSA_bio1_1981_1990_50km.tif"))
bioclim1_rec <- rast(file.path(data_dir, "Bioclim_2009_2018", "CHELSA_bio1_2009_2018_50km.tif"))
...
length(unique(BS_subset$Species)) # 687 species

# merge with EBBA and BBS species names:
...
BS_subset %>% 
  filter(Species %in% sel_species) %>% View

sel_species[which(!sel_species %in% BS_subset$Species)] # 39 of selected 117 EBBA species not immediately found in BioShifts
sel_species[which(sel_species %in% BS_subset$Species)] 

# BBS:
# species selection: 
BBS_sel_species <- read.csv(file = file.path(data_dir, "species_stability_contUS_BL22_120423.csv")) %>% 
  filter(stability >= 0.5) %>% 
  pull(species) %>% 
  sort # 297

BBS_sel_species[which(!BBS_sel_species %in% BS_subset$Species)] # 41 of selected 297 BBS species not immediately found in BioShifts

BS_subset %>% 
  filter(Species %in% BBS_sel_species) %>% View


BS_Table_S1 <- read.csv(file.path(data_dir, "BioShifts", "Lenoir_et_al", "Lenoir_et_al", "Analysis", "Table_S1.csv"),
                        sep = ";")

# match EBBA / BBS species with BioShifts and References
# matching with Polygon name possible? no, but information if a species showed range shift elsewhere may also be interesting