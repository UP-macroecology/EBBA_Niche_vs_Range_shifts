...
##Read in EBBA data##- UNLESS EDITTING CONVERSIONS, SKIP! Reading in and processing the large shapefiles takes too long!
...
#Unify taxonomy between EBBA's based on EBBA methods chapter-Removing all species from Table 6 unless necessary taxon changes clear!
...
#Remove non-comparable species from EBBA2 (and some from EBBA1)
...
#Get unique species names and counts, then remove all species with less than 20 records
...
EBBA1_Dropped <- unique(EBBA1_Dropped$species)#70 species dropped

#subset EBBA1 to include ONLY species with records in EBBA2
...
EBBA2_Dropped <- unique(EBBA2_Dropped$brdlf__)#160 species dropped
...
#Removing seabird/coastal breeding species#
