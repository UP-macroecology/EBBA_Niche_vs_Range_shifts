...
# EBBA 2 grid filtered by comparable cells:
EBBA_grid_comp <- read_sf(file.path(datashare_EBCC, "EBBA2", "ebba2_grid50x50_v1", "ebba2_grid50x50_v1.shp")) %>% 
  filter(cell50x50 %in% EBBA1_cells$cell50x50)

# join richness by cell ID:
EBBA_richness_grid <- EBBA_grid_comp %>% 
  ...
  mutate(diff2 = ifelse(diff > quantile(diff, 0.975), quantile(diff, 0.975), diff)) %>% # set values below -30 and above 30 to -30 and 30, respectively, to avoid colour scale being dominated by outliers
  mutate(diff2 = ifelse(diff < -quantile(diff, 0.975), -quantile(diff, 0.975), diff2)) 
...
ggplot() +
  ...
  scale_fill_viridis_c("richness EBBA 1")

# EBBA 2 richness:
ggplot() +
  geom_sf(data = EBBA_richness_grid, aes(fill = EBBA2_richness), colour = NA) +
  scale_fill_viridis_c("richness EBBA 2")

# difference:
diffplot <- ggplot() +
  geom_sf(data = EBBA_richness_grid, aes(fill = diff2), colour = NA) +
  #theme(panel.background = element_rect(fill = "#BFD5E3")) + # #BFD5E3
  theme_dark() +
...
# load results analyses results (output of 4_niche_shift_analysis.R and 4_range_shift_analysis.R)
if(env_background_species_specific){
  niche_results_ss <- read.csv(file = file.path(EBBA_data_dir, "niche_shift_results_env_species_spec.csv"))
  range_results_ss <- read.csv(file = file.path(EBBA_data_dir, "range_shift_results_env_species_spec.csv"))
  } else {
  niche_results_ss <- read.csv(file = file.path(EBBA_data_dir, "niche_shift_results_env_EBBA.csv"))
  range_results_ss <- read.csv(file = file.path(EBBA_data_dir, "range_shift_results_env_EBBA.csv"))
  }

#names(niche_results_ss)[c(32:36)] <- c("niche_stability_std", "niche_unfilling_std", "niche_expansion_std", "niche_abandonment_std", "niche_pioneering_std") # xx
#range_results_ss$range_abandonment_std <- NULL #xx
#range_results_ss$range_pioneering_std <- NULL #xx

# species (%) with significant dynamic values:
## niche:
niche_test_sign <- niche_results_ss %>% 
...
## range:
range_test_sign <- range_results_ss %>% 
  select(c(species, matches("_p_"))) %>% 
  summarise(across(.cols = where(is.numeric), 
                   .fns = ~length(which(.x <= 0.05)), 
                   .names = "{.col}_n_sig")) %>% 
...
niche_range_ss_df <- niche_results_ss %>% 
  left_join(range_results_ss, by = "species", suffix = c("_niche", "_range")) %>% 
...
# plot:
p <- ggplot(niche_range_ss_df, aes(x = category, y = value*100, fill = metric)) + # aes given inside ggplot() necessary for geom_text
  geom_boxplot(lwd = 0.1, outlier.size = 1, outlier.colour = "grey30", width = 0.9,
...
for(s in 1:2){#length(sel_species)){
  ...
  map <- ggplot(world) +
    ...
    geom_sf(data = EBBA_grid_comp, colour = "black", size = 1) +
