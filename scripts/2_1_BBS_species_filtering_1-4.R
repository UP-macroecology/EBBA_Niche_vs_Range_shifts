...
# which historic time period should be used (V1: 1980-1982, V2: 1996-1998)
historic <- 1996:1998 # 1980:1982 or 1996:1998

# read prepared BBS data (output of 1_BBS_prep_data.R):

if(all(historic == 1980:1982)){
  hist_prep_sf <- st_read(file.path(data_dir, "BBS_historic1.shp")) # output of 1_BBS_prep_data.R
  rec_prep_sf <- st_read(file.path(data_dir, "BBS_recent1.shp")) # output of 1_BBS_prep_data.R
}
if(all(historic == 1996:1998)){
  hist_prep_sf <- st_read(file.path(data_dir, "BBS_historic2.shp")) # output of 1_BBS_prep_data.R
  rec_prep_sf <- st_read(file.path(data_dir, "BBS_recent2.shp")) # output of 1_BBS_prep_data.R
}

# number of validation routes:
n_valid_routes <- length(unique(hist_prep_sf$RTENO)) # V1: 544; V2: 933
...
length(unique(hist_prep_df$species)) # 470 species for historic time period 1980-1983 (V1); 513 for historic time period 1996-1998 (V2) 
...
length(unique(rec_prep_df$species)) # 495 species in recent time period (V1), 525 (V2)
...
length(unique(hist_prep_df$species)) # 461 species left (V1: 9 species removed); 505 (V2, 8 species removed)
length(unique(rec_prep_df$species)) # 485 species left (V1: 10 species removed); 518 (V2, 7 species removed)
...
length(unique(hist_prep_df$species)) # 235 species left (V1); 326 (V2)
...
length(unique(rec_prep_df$species)) # 244 species left (V1); 337 (V2)
...
length(unique(hist_prep_df$species)) # 228 species left (V1); 322 (V2)

rec_prep_df <- rec_prep_df %>% 
  filter(n_occurrences <= 0.9 * n_valid_routes)

length(unique(rec_prep_df$species)) # 240 species left (V1); 334 (V2)
...
length(unique(hist_prep_df$species)) # 219 species left (V1); 314 (V2)

rec_prep_df <- rec_prep_df %>% 
  filter(species %in% hist_prep_df$species)

length(unique(rec_prep_df$species)) # 219 species left (V1); 314 (V2)

## 5. remove records where information is not on species level:
hist_prep_df <- hist_prep_df %>% 
  filter(!grepl(pattern = "sp\\.|/", x = species))

length(unique(hist_prep_df$species)) # 219 (V1); 312 (V2)

rec_prep_df <- rec_prep_df %>% 
  filter(!grepl(pattern = "sp\\.|/", x = species))

length(unique(rec_prep_df$species)) # 219 (V1); 312 (V2)

## how often does each species occur:
hist_prep_df %>%
  arrange(-n_occurrences) %>%
  distinct(species, n_occurrences, English_Common_Name)
